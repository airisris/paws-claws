<?php
  ...
  $sql = "SELECT posts.*, posts.id AS post_id, favourites.*, categories.name AS category_name 
    FROM favourites JOIN posts ON posts.id = favourites.post_id AND favourites.user_id = :user_id
...
<div class="col-9 p-0 m-0">
  <div class="p-3 ps-4 bg-white rounded me-4 position-relative" style="margin-top:100px;">
    <!-- tag -->
    <a href="/favourites"><button class="px-4 py-1 position-absolute rounded-top bg-white border-0" style="top:-32px;">Favourites</button></a>
    <!-- message -->
      ...
      <div class="col-4 p-0 mx-0 my-3">
        <div class="card p-0 m-0 overflow-hidden post-card" style="width: 17rem;">
          ...
          <div class="card-body position-relative">
            ...
            <a href="/post?id=<?= $post["post_id"]; ?>" class="text-decoration-none text-black"><button class="rounded p-2" style="width: 15rem;">View More</button></a>
            <!-- favourite icon -->
            <form method="GET" action="">
              <div class="position-absolute" style="bottom: 150px; right: 15px;">
                <?php if ($post["favourite"] == 1) { ?>
                  <a href="/post/favourite?id=<?= $post['post_id']; ?>">
                ...
                <?php } else { ?>
                  ...
                  <?php else : ?>
                    <i disabled class="bi bi-suit-heart-fill fs-3 text-secondary"></i></a>
                  <?php endif ; ?>
                <?php } ?>
              </div>
            </form>
